...
const getBookedCount = (schedule: ISchedule) => {
    return (
        schedule.doctorSchedules?.filter((item) => item.isBooked).length ?? 0
    );
};
...
export const schedulesColumns: ColumnDef<ISchedule>[] = [
    ...
    },
    {
        ...
        enableSorting: false,
        cell: ({ row }) => {
            const totalDoctorSchedules =
                row.original.doctorSchedules?.length ?? 0;
            return (
                <Badge variant="secondary">{totalDoctorSchedules} linked</Badge>
            );
        },
    },
    {
        ...
        cell: ({ row }) => {
            return (
                <span className="text-sm">{getBookedCount(row.original)}</span>
            );
        },
    },
    {
